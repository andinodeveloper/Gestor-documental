...
14. Vectorizar automáticamente los documentos oficializados para habilitar consulta inteligente mediante IA.
15. Permitir consultas con IA sobre documentos oficiales, generando respuestas con referencias, citas y enlaces directos a los documentos fuente.
16. Implementar permisos granulares de lectura por documento y versión, aplicables tanto a la consulta directa como a la consulta mediante IA.
17. Permitir subclasificar usuarios lectores en grupos o perfiles configurables según su rol dentro de la empresa, por ejemplo: directivos, administrativos, operativos, auditoría, soporte, tiendas, centros de distribución u otros.
18. Aplicar una misma capa centralizada de autorización para búsquedas, visualización de documentos, apertura de enlaces, recuperación vectorial, respuestas de IA y descarga de archivos.
...
- Búsqueda semántica/vectorial: `pgvector` sobre PostgreSQL o una capa equivalente preparada para migrar a Qdrant, Pinecone, Weaviate, Azure AI Search u otro motor vectorial.
- ORM: `Prisma`.
...
- IA: implementar una capa desacoplada para embeddings y modelo conversacional, configurable por variables de entorno, evitando acoplar el sistema a un único proveedor. Debe poder integrarse posteriormente con OpenAI, Azure OpenAI, Gemini, modelos locales u otro proveedor compatible.
...
2. **Seguridad documental y permisos granulares**
   - Administración de roles globales.
   - Administración de grupos/perfiles de lectores.
   - Asignación de usuarios a uno o varios grupos de lectura.
   - Definición de permisos por documento, versión, rol, grupo de lectores o usuario específico.
   - Restricción diferenciada para consulta directa, consulta por IA, consulta de historial y descarga.
   - Reglas de autorización con prioridad de denegación explícita cuando aplique.
...
5. **Solicitudes de elaboración o actualización**
...
6. **Flujo de revisión, aprobación y oficialización**
   - Carga de borrador.
...
11. **Consulta documental con IA**
   - Extracción de texto de documentos oficializados.
   - Fragmentación del contenido en bloques consultables.
   - Generación de embeddings/vectorización de documentos oficiales.
   - Consulta semántica mediante IA usando recuperación aumentada por contexto documental.
   - Respuestas con referencias obligatorias a documentos fuente.
   - Enlaces directos hacia la vista autorizada del documento oficial.
   - Filtros por proceso, tipo documental, código, estado, versión y vigencia.
   - Reindexación automática cuando se oficialice una nueva versión.
   - Auditoría de consultas, fuentes utilizadas y documentos consultados por IA.
...
El rol lector no debe entenderse como un grupo único y homogéneo. Debe permitir subclasificaciones o perfiles de lectura para controlar con mayor precisión qué documentos puede consultar cada usuario, según su función dentro de la empresa.

Debe poder:

- Ver únicamente documentos oficiales vigentes a los que tenga acceso.
- Leer documentos oficiales desde la aplicación.
- Buscar documentos oficiales autorizados.
- Consultar datos básicos del documento.
- Consultar mediante IA únicamente documentos autorizados para su usuario, rol o grupo lector.
...
- `documents.access.audit.read`
- `ai.query.official_documents`
- `ai.query.restricted_documents`
- `ai.query.confidential_documents`
- `ai.index.manage`
- `ai.index.reprocess`
- `ai.query.logs.read`
...
### 3.4 Modelo de autorización documental granular

La aplicación debe implementar un modelo combinado de permisos:

- **RBAC**: permisos por rol global, por ejemplo administrador, editor o lector.
- **Grupos/perfiles de lectores**: agrupación funcional de usuarios lectores para facilitar permisos masivos.
- **ACL documental**: reglas específicas de acceso por documento o versión documental.
- **Excepciones por usuario**: permisos o restricciones puntuales para personas concretas.

La autorización debe evaluarse antes de cualquier acceso al contenido documental. Esto aplica a:

- Listados y resultados de búsqueda.
- Árbol documental.
- Vista directa del documento.
- Vista de metadatos sensibles.
- Historial de versiones.
- Descarga.
- Recuperación de fragmentos vectorizados para IA.
- Respuestas de IA y enlaces citados.

La consulta directa y la consulta por IA deben compartir una misma capa de autorización. No debe existir una lógica separada que pueda provocar que un documento restringido no aparezca en el explorador, pero sí sea recuperado por IA.

#### Niveles sugeridos de visibilidad documental

Cada documento oficial debe poder clasificarse, como mínimo, en uno de estos niveles:

- `PUBLIC_INTERNAL`: visible para todos los usuarios internos con rol lector activo.
- `RESTRICTED_BY_GROUP`: visible solo para grupos/perfiles de lectores autorizados.
- `RESTRICTED_BY_ROLE`: visible solo para roles autorizados.
- `RESTRICTED_BY_USER`: visible solo para usuarios específicos.
- `CONFIDENTIAL`: visible solo para usuarios, grupos o roles explícitamente autorizados; no debe heredarse por defecto.
- `ARCHIVED_RESTRICTED`: visible solo para roles autorizados a consultar histórico u obsoletos.

#### Regla de prioridad

La evaluación de permisos debe seguir esta prioridad sugerida:

1. Usuario inactivo: denegar.
2. Documento no oficial o no vigente: denegar a lectores, salvo permiso especial.
3. Denegación explícita por usuario, grupo o rol: denegar.
4. Permiso explícito por usuario: permitir según alcance.
5. Permiso por grupo/perfil lector: permitir según alcance.
6. Permiso por rol: permitir según alcance.
7. Visibilidad interna general: permitir solo si el usuario tiene permiso global de lectura.
8. Si no existe una regla aplicable: denegar por defecto.

### 3.5 Subclasificación de usuarios lectores

Debe existir un catálogo administrable de grupos o perfiles de lectores. Estos grupos no sustituyen los roles globales del sistema; funcionan como una capa adicional para definir alcance de consulta documental.

Ejemplos de grupos iniciales sugeridos:

- `DIRECTIVO`: usuarios de dirección o alta gerencia.
- `GERENCIAL`: gerencias y jefaturas autorizadas.
- `ADMINISTRATIVO`: usuarios de áreas administrativas.
- `OPERATIVO`: usuarios operativos.
- `TIENDAS_SALAS`: usuarios relacionados con salas de venta.
- `CENTROS_DISTRIBUCION`: usuarios de centros de distribución y logística.
- `AUDITORIA_CONTROL`: usuarios de auditoría, control interno o cumplimiento.
- `SOPORTE_TECNOLOGICO`: usuarios de tecnología y soporte.
- `EXTERNO_AUTORIZADO`: usuarios externos con acceso limitado, si en el futuro se habilita.

Reglas mínimas:

- Un usuario puede pertenecer a uno o varios grupos lectores.
- Un documento puede autorizarse para uno o varios grupos lectores.
- Un documento puede restringirse para uno o varios grupos lectores.
- Las restricciones explícitas deben prevalecer sobre autorizaciones generales.
- La pertenencia a grupos debe ser auditable.
- Los permisos por grupo deben poder tener vigencia, fecha de inicio y fecha de fin cuando aplique.
- La lectura directa y la lectura mediante IA deben poder habilitarse o restringirse de forma independiente, aunque por defecto deben respetar la misma regla base de acceso.
...
> Nota: en una imagen se observa `ESQUEMAS DOCUMENTALES - ESQ`, pero en el archivo de tipos documentales aparece el código `ESD`. Para la primera versión se debe usar `ESD`, salvo que el usuario confirme que el código correcto es `ESQ`.
...
### 14.3 Permisos granulares para consulta directa

La consulta documental directa debe filtrar los documentos antes de mostrarlos al usuario. Un usuario lector no debe ver en resultados, árbol documental, tarjetas, metadatos sensibles ni enlaces de apertura los documentos para los cuales no tenga permiso.

Requisitos mínimos:

- Aplicar permisos por documento y, cuando corresponda, por versión documental.
- Permitir reglas de acceso por rol, grupo/perfil lector y usuario específico.
- Permitir restricciones explícitas para ocultar documentos a usuarios o grupos concretos aunque tengan un rol lector general.
- Separar el permiso de lectura del permiso de descarga. Poder leer no implica poder descargar.
- Separar el permiso de lectura vigente del permiso de consulta histórica. Poder leer la versión vigente no implica poder consultar versiones anteriores u obsoletas.
- Registrar auditoría cuando se creen, modifiquen o revoquen reglas de acceso.
- Registrar auditoría de accesos a documentos restringidos o confidenciales.

### 14.4 Consulta documental con IA

La aplicación debe incluir una capacidad escalable de consulta inteligente sobre documentos oficializados.

El objetivo es que el usuario pueda hacer preguntas en lenguaje natural y que el sistema responda utilizando únicamente el contenido de documentos oficiales a los que el usuario tenga permiso de acceso. La respuesta debe ayudar al usuario a encontrar información, pero también debe mostrar los documentos que sustentan la respuesta y permitir abrirlos mediante enlaces internos.

La consulta con IA debe seguir un enfoque tipo RAG (`Retrieval Augmented Generation`):

1. El usuario realiza una pregunta.
2. El sistema valida los permisos del usuario.
3. El sistema busca fragmentos relevantes en el índice vectorial, aplicando filtros de seguridad, estado, proceso, tipo documental y versión vigente.
4. El sistema envía al modelo IA únicamente los fragmentos recuperados y metadatos necesarios.
5. El modelo genera una respuesta basada en las fuentes recuperadas.
6. La respuesta muestra referencias, documentos fuente y enlaces internos hacia la vista autorizada del documento.

La IA no debe responder como si tuviera certeza cuando no existan documentos suficientes. En esos casos debe indicar que no encontró información oficial suficiente y sugerir filtros o documentos relacionados.

### 14.5 Alcance de la vectorización documental

La vectorización debe aplicarse inicialmente solo a documentos oficializados.

Reglas mínimas:

- No vectorizar borradores por defecto.
- No exponer documentos en revisión dentro de la consulta IA para usuarios lectores.
- Vectorizar la versión oficial vigente de cada documento.
- Mantener trazabilidad de versiones vectorizadas.
- Cuando se oficialice una nueva versión, la versión anterior debe dejar de ser la fuente principal de consulta para lectores, salvo que el usuario tenga permiso para consultar historial.
- Cuando un documento pase a obsoleto, sus vectores deben marcarse como inactivos para consulta general o mantenerse consultables solo para roles autorizados.
- Cada fragmento vectorizado debe conservar vínculo con documento, versión, página/sección aproximada y metadatos.
- La consulta IA debe devolver enlaces internos a la vista del documento, no enlaces directos de descarga, salvo que el usuario tenga permiso de descarga.

### 14.6 Pipeline sugerido de indexación IA

El pipeline de indexación debe ejecutarse al oficializar un documento y también permitir reprocesamiento manual por un administrador.

Flujo sugerido:

1. **Documento oficializado**: se marca la versión como oficial y vigente.
2. **Extracción de texto**: el sistema extrae contenido del archivo oficial.
3. **Normalización**: se limpian encabezados repetitivos, saltos innecesarios, caracteres inválidos y contenido no útil.
4. **Fragmentación**: el texto se divide en bloques controlados, conservando metadatos de documento, versión, sección, página o rango aproximado.
5. **Generación de embeddings**: cada bloque se convierte en vector mediante el proveedor configurado.
6. **Almacenamiento vectorial**: se guardan vectores y metadatos en PostgreSQL con `pgvector` o motor vectorial equivalente.
7. **Activación del índice**: los fragmentos quedan disponibles para consulta solo después de finalizar correctamente el proceso.
8. **Auditoría**: se registra fecha, usuario/proceso que disparó la indexación, modelo usado, estado y errores si existieron.

### 14.7 Seguridad y gobernanza de la IA

La IA debe respetar las mismas reglas de acceso documental que el explorador tradicional.

Requisitos mínimos:

- Aplicar control de permisos antes de recuperar fragmentos para IA.
- Usar la misma capa de autorización que se utiliza para la consulta directa del documento.
- Filtrar por usuario, rol, grupo/perfil lector, estado, vigencia, versión y reglas explícitas de denegación antes de construir el contexto del modelo.
- No enviar al modelo contenido de documentos que el usuario no puede consultar.
- No mostrar como fuente documentos que el usuario no pueda abrir en la vista controlada de la aplicación.
- Incluir instrucciones de sistema para que el modelo responda solo con base en documentos recuperados y autorizados.
- Mostrar fuentes utilizadas en cada respuesta.
- Registrar consulta, usuario, fecha, filtros usados, documentos recuperados y documentos citados.
- Permitir desactivar temporalmente la consulta IA sin afectar el repositorio documental.
- Permitir cambiar proveedor/modelo de IA sin rediseñar la aplicación.
- No usar la IA como sustituto de la aprobación documental; la IA solo consulta, resume y orienta.
...
- Creación de usuario.
- Cambio de permisos.
- Asignación o retiro de usuarios en grupos/perfiles lectores.
- Creación, modificación o revocación de reglas de acceso documental.
- Acceso a documentos restringidos o confidenciales.
- Intentos denegados de acceso directo o por IA.
- Creación de solicitud.
...
Catálogo administrable de grupos o perfiles de lectores para controlar permisos de consulta documental por segmento funcional.
...
Relación muchos-a-muchos entre usuarios y grupos/perfiles lectores.
...
Política principal de visibilidad de un documento o versión documental.

- `id`
- `documentId`
- `documentVersionId` opcional
- `visibilityLevel` (`PUBLIC_INTERNAL`, `RESTRICTED_BY_GROUP`, `RESTRICTED_BY_ROLE`, `RESTRICTED_BY_USER`, `CONFIDENTIAL`, `ARCHIVED_RESTRICTED`)
- `defaultDirectReadAllowed`
- `defaultAiQueryAllowed`
- `defaultHistoryReadAllowed`
...
Regla granular de autorización o restricción por documento, versión, usuario, rol o grupo lector.

- `id`
- `documentId`
- `documentVersionId` opcional
- `subjectType` (`USER`, `ROLE`, `READER_GROUP`)
- `subjectId`
- `accessScope` (`DIRECT_READ`, `AI_QUERY`, `HISTORY_READ`, `DOWNLOAD`, `METADATA_READ`)
- `effect` (`ALLOW`, `DENY`)
- `validFrom`
- `validUntil`
- `reason`
...
Permisos granulares excepcionales por usuario y documento. Puede mantenerse para compatibilidad o implementarse como una vista/capa simplificada sobre `DocumentAccessRule` cuando `subjectType = USER`.
...
### 16.9 Entidades para IA, extracción de texto y vectorización

#### `DocumentTextExtraction`

Registro del texto extraído desde una versión documental oficial.
...
- `documentFileId`
- `extractionStatus`
- `extractedText` o `extractedTextStoragePath`
- `language`
- `textChecksum`
- `errorMessage`
- `extractedAt`
...
Fragmento textual utilizado para búsqueda semántica y respuesta con IA.
...
Vector asociado a cada fragmento textual.
...
> Nota técnica: si se usa PostgreSQL, preferir `pgvector` para almacenar `embeddingVector`. Si el ORM no soporta directamente el tipo vectorial, dejar migración SQL manual y capa de repositorio especializada.
>
> Nota de seguridad: la búsqueda vectorial no debe depender únicamente de metadatos embebidos en el vector. Antes de responder, el sistema debe validar contra las reglas vigentes de `DocumentAccessPolicy` y `DocumentAccessRule`, para evitar que cambios recientes de permisos dejen contenido expuesto por índices antiguos.

#### `AIIndexJob`

Proceso de extracción, fragmentación y vectorización.
...
Registro de una consulta realizada por un usuario a la IA.
...
Fuente documental utilizada o citada en una respuesta IA.
...
#### `AIProviderConfig` opcional

Configuración administrable del proveedor IA si no se desea manejar únicamente por variables de entorno.
...
### 17.8 Consulta documental con IA

- `/ai/search`
  - Caja de consulta en lenguaje natural.
  - Filtros por proceso, tipo documental, estado, versión, fecha y vigencia.
  - Respuesta generada por IA.
  - Lista de documentos fuente con enlace interno.
  - Fragmentos o citas utilizadas.

- `/admin/ai/index`
  - Estado de indexación por documento y versión.
  - Reprocesamiento manual de documentos.
  - Historial de errores de extracción/vectorización.
  - Configuración visible del proveedor/modelo activo, sin exponer claves secretas.

- `/admin/ai/logs`
  - Historial de consultas IA.
  - Usuario consultante.
  - Pregunta realizada.
  - Documentos recuperados/citados.
  - Fecha y estado de respuesta.
...
17. La IA solo debe consultar documentos oficializados y autorizados para el usuario.
18. La IA debe responder con fuentes; si no hay fuentes suficientes, debe indicarlo claramente.
19. Al oficializar una nueva versión, debe ejecutarse o programarse la vectorización de esa versión.
20. Al archivar/volver obsoleto un documento, sus vectores deben actualizarse para evitar que aparezca como vigente.
21. Los enlaces generados por IA deben abrir la vista controlada del documento dentro de la aplicación, no el archivo físico directo.
22. Las consultas IA y sus fuentes deben quedar auditadas.
23. Los permisos de consulta directa, consulta por IA, historial y descarga deben evaluarse como alcances separados.
24. Un documento restringido para un usuario no debe aparecer en buscadores, árbol documental, respuestas de IA ni enlaces de fuentes.
25. La denegación explícita por usuario, rol o grupo lector debe prevalecer sobre permisos generales.
26. Un usuario lector puede pertenecer a múltiples grupos/perfiles, y el acceso final debe resolverse combinando todos sus permisos y restricciones vigentes.
27. Cuando se modifique una regla de acceso documental, el cambio debe aplicar inmediatamente a la consulta directa y a la consulta por IA, aunque los vectores ya existan.
...
### 19.4 Historial de consulta IA

Exportar para usuarios autorizados:
...
16. La indexación/vectorización debe ejecutarse de forma asíncrona para no bloquear la oficialización.
17. Debe existir una estrategia de reprocesamiento ante fallos de extracción, cambios de modelo de embeddings o cambios de proveedor.
18. La arquitectura de IA debe ser desacoplada y configurable por variables de entorno.
19. La búsqueda IA debe aplicar control de acceso a nivel de consulta y no depender solo de filtros visuales.
20. Debe contemplarse protección contra respuestas sin fuente, prompt injection desde documentos y exposición accidental de información no autorizada.
21. Debe registrarse observabilidad básica de la IA: errores, latencia, documentos recuperados y modelo usado.
...
14. `.env.example`.
15. Datos de prueba.
16. Pipeline inicial de extracción de texto y vectorización para documentos oficializados.
17. Módulo de consulta documental con IA basado en documentos oficiales.
18. Pantalla administrativa para revisar estado de indexación/reprocesamiento.
19. Registro/auditoría de consultas IA y documentos fuente utilizados.
...
### 22.8 Consulta documental con IA

- Dado un documento oficializado, cuando finaliza la oficialización, entonces el sistema debe crear o programar un trabajo de extracción/vectorización.
- Dado un documento oficial vectorizado, cuando un lector autorizado pregunta por su contenido, entonces la IA puede usarlo como fuente y mostrar enlace interno al documento.
- Dado un usuario sin permiso para ver un documento, cuando realiza una consulta IA relacionada, entonces el sistema no debe recuperar ni citar ese documento.
- Dado que no existen fuentes suficientes para responder, cuando el usuario realiza una consulta, entonces la IA debe indicar que no encontró información oficial suficiente.
- Dado que una nueva versión es oficializada, cuando se consulta por IA, entonces la versión vigente debe tener prioridad sobre versiones anteriores.
- Dado un documento archivado u obsoleto, cuando consulta un lector general, entonces la IA no debe presentarlo como documento vigente.
- Dado que la IA responde una pregunta, entonces debe mostrar al menos una fuente documental cuando haya utilizado contenido del repositorio.
- Dado un documento autorizado solo para directivos, cuando un usuario operativo pregunta por su contenido mediante IA, entonces la IA no debe recuperar, citar ni sugerir ese documento.
- Dado que se revoca el permiso de un grupo lector sobre un documento, cuando un usuario de ese grupo consulta por IA, entonces el documento debe quedar excluido inmediatamente aunque sus embeddings sigan almacenados.
- Dado que una respuesta IA muestra fuentes, cuando el usuario abre un enlace, entonces debe pasar por la vista controlada y volver a validar permisos.
...
### 23.3 Grupos/perfiles lectores sugeridos

```json
[
  { "code": "DIRECTIVO", "name": "Directivo" },
  { "code": "GERENCIAL", "name": "Gerencial / Jefatura" },
  { "code": "ADMINISTRATIVO", "name": "Administrativo" },
  { "code": "OPERATIVO", "name": "Operativo" },
  { "code": "TIENDAS_SALAS", "name": "Tiendas / Salas de venta" },
  { "code": "CENTROS_DISTRIBUCION", "name": "Centros de distribución y logística" },
  { "code": "AUDITORIA_CONTROL", "name": "Auditoría, control interno o cumplimiento" },
  { "code": "SOPORTE_TECNOLOGICO", "name": "Soporte tecnológico" },
  { "code": "EXTERNO_AUTORIZADO", "name": "Externo autorizado" }
]
```
...
7. Si la aprobación requiere firma electrónica, firma digital o solamente aprobación registrada en el sistema.
8. Si se enviarán notificaciones por correo.
9. Si habrá fechas de vigencia, vencimiento y alertas de revisión periódica.
10. Si los permisos de descarga aprobados tendrán vencimiento obligatorio.
11. Si los documentos obsoletos serán visibles para lectores o solo para administradores/editores.
12. Si se requiere multicompañía, multiárea o separación por unidades organizativas.
13. Si se requiere importación masiva inicial de documentos existentes.
14. Si se requiere integración con un sistema documental existente.
15. Proveedor inicial para IA y embeddings: OpenAI, Azure OpenAI, Gemini, modelo local u otro.
16. Motor vectorial inicial: PostgreSQL + pgvector, Qdrant, Pinecone, Weaviate, Azure AI Search u otro.
17. Formatos iniciales que deberán soportar extracción de texto para IA: PDF, DOCX, XLSX, PPTX u otros.
18. Si se permitirá consultar documentos obsoletos mediante IA para administradores/editores o si quedarán fuera del índice activo.
19. Política de retención de logs de consultas IA y respuestas generadas.
...
16. Implementar extracción de texto de documentos oficiales.
17. Implementar vectorización e índice semántico.
18. Implementar consulta IA con fuentes y enlaces internos.
19. Implementar administración de indexación y logs IA.
